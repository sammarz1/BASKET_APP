...
from basket_app.core.config import StorageSettings
...
def test_clean_endpoint_url_empty_string():
    settings = StorageSettings(s3_endpoint_url="  ")
    assert settings.s3_endpoint_url is None


def test_clean_endpoint_url_valid():
    settings = StorageSettings(s3_endpoint_url="http://localhost:9000")
